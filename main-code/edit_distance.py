...
class SequenceMatcher(object):
    ...
    def __init__(self, a=None, b=None, test=operator.eq,
                 action_function=lowest_cost_action, max_error=float('inf')):
    ...
    def _compute_distance_fast(self):
        ...
        d, m = edit_distance(self.seq1, self.seq2,
                             action_function=self.action_function,
                             test=self.test, max_error=self.max_error)
        if self.dist:
            assert d == self.dist
        if self._matches:
            assert m == self._matches
...
def edit_distance(seq1, seq2, action_function=lowest_cost_action, test=operator.eq, max_error=float('inf')):
    ...
    for i in range(1, m + 1):
		...
		count = count + 1
            ...
            if count > max_error : return count, -1
